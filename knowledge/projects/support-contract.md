# Support Contract

## Context
General support contract for bug investigation and fixes.


## Timeline / Log
### 2026-02-20
- Harvest time logged for Sales Order due date fix/deployment, environment data updates/cleanup, and admin checks.

### 2026-02-20
- Harvest time logged today: Sales Order due date fix deployment; adding records in dev/upper envs; admin checks and ticket/thread updates.

### 2026-02-20
- Harvest time logged: Sales Order due date fix deployment; added records to dev/upper envs via DevOps and performed cleanup; admin updates.

### 2026-02-20
- Support work logged: Sales Order due date fix deployed; environment/user records updates via DevOps; admin checks and ticket/thread updates.

### 2026-02-20
- Harvest logged Sales Order due date fix deployment, record additions via DevOps, and admin/ticket updates. Adobe Sign signature request received for 'WA COMMUNICATIONS_SOW004_Support (T&M)_19 February 2026'.

### 2026-02-20
- Harvest: sales order due date fix deployment; adding records across environments via DevOps.

### 2026-02-20
- Harvest time logged: deployment fix for Sales Order DueDate and adding Umar records across environments; cleanup for constraints.

### 2026-02-20
- Harvest time logged for Sales Order due date fix deployment.

### 2026-02-20
- Harvest time logged (0.25h): Sales Order DueDate fix - deployment.


### 2026-02-18
- Bug investigation and fixes logged (4.25h) per Harvest.

---
*Last Updated: 2026-02-20*
